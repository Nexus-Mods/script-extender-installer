...
async function downloadScriptExtender(api: types.IExtensionApi, gameSupport: IGameSupport, gameId: string): Promise<void> {
  ...
  if (gameSupport.nexusMods) {
    ...
    return Promise.resolve({
      description: {
        ...
        long: `Vortex could not detect "{{name}}". This means it is either not installed or installed incorrectly.\n\n`
    ...
    return Promise.resolve({
      description: {
        short: `${gameSupport.name} not installed`,
        long: `Vortex could not detect ${gameSupport.name}. This means it is either not installed or installed incorrectly.\n\n`
